                ...
                <ul className="grid grid-cols-4 p-[32px_20px] gap-[32px] max-[1300px]:flex max-[1300px]:flex-wrap max-[1300px]:justify-evenly"> 
                    { restaurantData.map((item) => {
                            ...
                            <li  className="w-[100%] list-hover max-[1300px]:w-[278px]"  key={item.id}>
                                <Link href={`/collections/${id}/${item.title.toLowerCase().replace(/\s+/g,'-')}`} className={`flex flex-col gap-[12px]  whitespace-nowrap overflow-clip text-ellipsis ${item.id>=50 ? 'cursor-not-allowed': 'cursor-pointer'}`}>
